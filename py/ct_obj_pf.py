...
data = pd.read_csv(
    ...
    encoding='utf-8',
    sep='|',
    usecols=['obj', 'statement', 'label', 'book', 'page']
)
print(data.info())
...
for index, row in data.iterrows():
    ...
    if tmpno % 10000 == 0:
        print(tmpno)
    lineNo += 1
    lines.append("ars:pf_" + str(row['statement']) + " " + "dcterms:bibliographicCitation" + " " + "'" + str(row['book']) + ", " + str(row['page']) + "'" + ".")
    lines.append("ars:ic_" + str(row['obj']) + " " + "lado:representedBy" + " " + "ars:pf_" + str(row['statement']) + ".")
    lines.append("ars:pf_" + str(row['statement']) + " " + "lado:hasImage" + " " + "<" + "https://zenodo.org/record/5645180/files/" + str(row['obj']) + ".jpg>" + ".")
    lines.append("")
